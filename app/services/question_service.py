import uuid
import traceback
from app.api.parse_routes import parse_audio_video, parse_document, parse_web_url, parse_youtube
from sqlalchemy.orm import Session, joinedload
from fastapi import HTTPException
from typing import Dict, Any
from app.models import QuestionGen, QuestionAnswerGen, Flashcard
import logging
from app.config import current_date_time
from app.processor.question_generator import question_generator
from request_response.question import (
    DocumentGenerationRequest,
    DocumentFileUploadRequest,
    DocumentUrlUploadRequest,
)

from app.processor.question_generator import Quest
...
from utils.helper import (detect_url_type, detect_file_type)
...
class QuestionService:
    ...
    def get_questions_by_document(db, document_id: str):
        return (
            db.query(QuestionGen)
            .options(joinedload(QuestionGen.question_answers))
            .filter(QuestionGen.document_id == document_id)
            .all()
        )
    ...
    def delete_questions_by_document(db: Session, document_id: str):
        db.query(QuestionGen).filter(QuestionGen.document_id == document_id).delete()
        db.commit()
    ...
    def process_rag_quiz_and_flashcards(self, request: DocumentGenerationRequest, db: Session) -> Dict[str, Any]:
        try:
            ...
            generation_record = QuestionGeneration(
                ...
                generation_parameters={
                    ...
                    "model": question_generator.model_name,
                    "document_ids": request.document_ids,
                },
                model_version=question_generator.model_name,
                generation_status="processing",
            )
            db.add(generation_record)
            db.flush()

            quiz_questions = question_generator.generate_rag_quiz(request.topic, relevant_context, request.quiz_count)
            flashcards = question_generator.generate_rag_flashcards(request.topic, relevant_context, request.flashcard_count)

            for q in quiz_questions:
                quality_score = question_generator.calculate_quality_score(
                    {"question": q.question, "answer": q.correct_answer, "explanation": q.explanation}, relevant_context
                )
                question_obj = QuestionGen(
                    content=q.question,
                    type=q.type,
    ...
    def generate_question_from_url_service(self, request: DocumentUrlUploadRequest, db: Session):
        try:
            ...
            if url_type == "youtube":
                document = parse_youtube(db, request.url, request.session_id)
            else:
                document = parse_web_url(db, request.url, request.session_id)

            document_id = str(document.id)
            with open(document.content_file_path, "r", encoding="utf-8") as f:
                text_content = f.read()

            vector_service.chunk_and_embed_document(db, document_id, text_content)
    ...
    def generate_question_from_file_service(self, request: DocumentFileUploadRequest, db: Session):
        try:
            ...
            if file_type == "audio_video":
                document = parse_audio_video(db, request.file, request.session_id)
            else:
                document = parse_document(db, request.file, request.session_id)
